...
2. **Create ModelServing**: Create a ModelServing CR with the `kthena.io/ranktable-template` annotation
3. **Controller Creates ConfigMaps**: The controller creates empty ranktable ConfigMaps for each role/group
4. **Controller Creates Pods**: Pods are created with init containers that wait for ranktable data
...
6. **Controller Generates Ranktables**: The controller watches pod annotations, parses them, and generates ranktables
7. **Init Containers Complete**: Once ranktables are ready, init containers complete and main containers start

### Key Features

- **Template-based**: Flexible Go templates for parsing and generating ranktables
- **Automatic Injection**: Init containers and volume mounts are automatically injected
...
- `kthena.io/ranktable-template`: **(Required)** Name of the ranktable template ConfigMap
...
3. **Init container waits**: The new pod's init container waits for ranktable data
...
6. **Init container completes**: Pod starts after ranktable is ready
...
3. **New pods are created**: Controller creates new pods with init containers
...
- **ConfigMap clearing**: Ranktable ConfigMaps are cleared to trigger init container wait
- **Synchronization**: Init containers ensure pods don't start until ranktable is ready
...
**Symptom**: Init container `wait-ranktable` keeps waiting
...
**Solutions**:
1. Verify ModelServing has the correct annotation
