...
`kubectl drain` 可能在很短时间内为多个节点、多个 Pod 并发发起 eviction 请求。由于 informer cache 看到 Pod deletion 存在延迟，Webhook 在允许驱逐一个逻辑单元后，需要立即把该 ServingGroup 或 role instance 从可用预算里扣除。

本设计使用每个 `ModelServing` 一个 ConfigMap 作为共享 tracker：

- ConfigMap 与 `ModelServing` 位于同一 namespace。
- ConfigMap 记录最近被允许驱逐的逻辑单元及过期时间。
- tracker TTL 默认 60s，只用于覆盖 informer cache lag。
- 多个 controller-manager 副本可同时处理 eviction webhook。
- 每次 allow 前通过 ConfigMap `resourceVersion` 执行 read-modify-write；若发生冲突则重试并基于最新 tracker 重新计算预算。

这样不需要将 controller-manager 强制为单副本，也不要求 admission Service 定向路由到 leader。
...
3.  **共享 tracker 设计**：通过 ConfigMap 记录已 allow 的逻辑中断单元，并使用 `resourceVersion` 冲突重试保证多副本并发安全。
