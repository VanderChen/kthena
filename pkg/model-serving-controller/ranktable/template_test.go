...
podName: {{ $data.pod_name | quote }}`,
				...
				client.CoreV1().ConfigMaps("kthena-system").Create(context.TODO(), parserCM, metav1.CreateOptions{})
				...
				client.CoreV1().ConfigMaps("kthena-system").Create(context.TODO(), templateCM, metav1.CreateOptions{})
				return client
			},
...
podName: {{ $data.pod_name | quote }}`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := tm.GetRanktableTemplate(context.TODO(), tt.templateName)
			if tt.wantErr {
				assert.Error(t, err)
				assert.Nil(t, got)
			} else {
