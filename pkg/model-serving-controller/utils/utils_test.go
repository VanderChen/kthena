...
func TestGenerateWorkerPod_WithAnnotations(t *testing.T) {
	...
	var pod *corev1.Pod
	assert.NotPanics(t, func() {
		pod = GenerateWorkerPod(role, ms, "test-group", 0, 1, "test-revision", "role-revision")
	})
	assert.NotNil(t, pod)
	assert.Equal(t, annotations, pod.Annotations)
}
...
func TestGeneratePodsDoNotInjectEntryAddress(t *testing.T) {
	...
	entryPod := GenerateEntryPod(role, ms, "test-ms-0", 0, "revision", "role-revision")
	workerPod := GenerateWorkerPod(role, ms, "test-ms-0", 0, 1, "revision", "role-revision")
