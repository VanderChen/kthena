...
func (c *ModelServingController) scaleUpServingGroups(ctx context.Context, ms *workloadv1alpha1.ModelServing, servingGroupList []datastore.ServingGroup, expectedCount int, newRevision string) error {
	...
	createServingGroup := func(ordinal int, revision string, roles []workloadv1alpha1.Role) error {
		roles = mergeLatestRoleReplicas(roles, ms.Spec.Template.Roles)
		...
		groupTemplate := ms.DeepCopy()
		groupTemplate.Spec.Template.Roles = roles
		if err := c.createOrUpdatePodGroupByServingGroup(ctx, groupTemplate, groupName); err != nil {
			return err
		}
			...
			if revisionToUse != "" {
				...
				if err != nil {
					...
					if oldRoles, err := utils.GetRolesFromControllerRevision(cr); err != nil {
						...
						klog.V(4).Infof("manageRole: using historical templates from revision %s with latest replica counts for protected ServingGroup %s", revisionToUse, servingGroup.Name)
					}
				} else {
		...
		for _, targetRole := range rolesToManage {
			if scaleDownOnly {
				...
				if targetRole.Replicas != nil && len(roles) > int(*targetRole.Replicas) {
					if err := c.scaleDownRoles(ctx, ms, servingGroup.Name, targetRole, roles, int(*targetRole.Replicas)); err != nil {
						return err
					}
				}
				continue
			}
			if err := c.manageRoleReplicas(ctx, ms, servingGroup.Name, targetRole, servingGroupOrdinal, revisionToUse); err != nil {
				return err
			}
		}
	}
...
// mergeLatestRoleReplicas keeps historical Role templates for partition-protected
// ServingGroups while applying the latest independently scalable replica counts.
func mergeLatestRoleReplicas(historicalRoles, latestRoles []workloadv1alpha1.Role) []workloadv1alpha1.Role {
	latestRolesByName := make(map[string]workloadv1alpha1.Role, len(latestRoles))
	for _, role := range latestRoles {
		latestRolesByName[role.Name] = role
	}

	mergedRoles := make([]workloadv1alpha1.Role, 0, len(historicalRoles))
	for i := range historicalRoles {
		role := historicalRoles[i].DeepCopy()
		if latestRole, ok := latestRolesByName[role.Name]; ok {
			if latestRole.Replicas == nil {
				role.Replicas = nil
			} else {
				replicas := *latestRole.Replicas
...
func (c *ModelServingController) scaleDownRoles(ctx context.Context, ms *workloadv1alpha1.ModelServing, groupName string, targetRole workloadv1alpha1.Role, roleList []datastore.Role, expectedCount int) error {
	...
	for i := 0; i < numToDelete; i++ {
		...
		if err := c.DeleteRole(ctx, ms, groupName, targetRole.Name, targetName); err != nil {
			return err
		}
	}
	return nil
...
func (c *ModelServingController) scaleUpRoles(ctx context.Context, ms *workloadv1alpha1.ModelServing, groupName string, targetRole workloadv1alpha1.Role, roleList []datastore.Role, expectedCount int, servingGroupOrdinal int, newRevision string) error {
	...
	for i := 0; i < toCreate; i++ {
		...
		if err != nil {
			return fmt.Errorf("create role %s for ServingGroup %s: %w", utils.GenerateRoleID(targetRole.Name, newIndex), groupName, err)
		} else {
...
func (c *ModelServingController) manageRoleReplicas(ctx context.Context, ms *workloadv1alpha1.ModelServing, groupName string, targetRole workloadv1alpha1.Role, servingGroupOrdinal int, newRevision string) error {
	...
	if err != nil {
		return fmt.Errorf("manageRoleReplicas: cannot get role %s in ServingGroup %s: %w", targetRole.Name, groupName, err)
	}
	...
	history := c.templates(ctx, ms)
	scaleUpTemplate, scaleUpRevision := targetRole, newRevision
	hasActiveTemplate, unresolved := false, false
	for _, roleObj := range roleList {
		...
		template, revision, hash, err := history.instanceTemplate(ctx, groupName, targetRole.Name, roleObj, pods)
		if err != nil {
			c.reportRevisionUnresolved(ms, groupName, err)
			unresolved = true
			continue
		}
		if !hasActiveTemplate {
			scaleUpTemplate, scaleUpRevision, hasActiveTemplate = template, revision, true
		} else if !utils.EqualRoleTemplate(scaleUpTemplate, template) {
			// Do not grow an already mixed Role type before rollout resolves it.
			unresolved = true
		}
		...
		if missing {
			klog.V(2).Infof("Restoring missing pods for role %s/%s in ServingGroup %s at revision %s", targetRole.Name, roleObj.Name, groupName, revision)
	...
	if len(roleList) < expectedCount {
		...
		if !hasActiveTemplate && (ms.Spec.RolloutStrategy == nil || ms.Spec.RolloutStrategy.Type == workloadv1alpha1.ServingGroupRollingUpdate) {
			// An empty Role in an existing SG may be recovering from RoleRecreate.
			// Only replacing the SG authorizes its new template in SG rollout mode.
			groupRevision, _ := c.store.GetServingGroupRevision(utils.GetNamespaceName(ms), groupName)
			template, revision, _, err := history.instanceTemplate(ctx, groupName, targetRole.Name, datastore.Role{Revision: groupRevision}, nil)
			if err != nil {
				c.reportRevisionUnresolved(ms, groupName, err)
				return nil
			}
			scaleUpTemplate, scaleUpRevision = template, revision
		}
		...
		scaleUpTemplate.Replicas = targetRole.Replicas
		if err := c.scaleUpRoles(ctx, ms, groupName, scaleUpTemplate, roleList, expectedCount, servingGroupOrdinal, scaleUpRevision); err != nil {
			return err
		}
		...
		if exists && len(outdatedRoleNames) > 0 {
			for _, roleName := range outdatedRoleNames {
				...
				for i := range outdatedRoles {
					if err := c.DeleteRole(ctx, ms, sg.Name, roleName, outdatedRoles[i].Name); err != nil {
						return updateCount, fmt.Errorf("delete outdated role %s/%s in ServingGroup %s: %w", roleName, outdatedRoles[i].Name, sg.Name, err)
					}
...
func (c *ModelServingController) handleReadyPod(ms *workloadv1alpha1.ModelServing, servingGroupName string, newPod *corev1.Pod) error {
	...
	if err != nil {
		...
		c.reportRevisionUnresolved(ms, servingGroupName, err)
	} else if roleReady {
	...
	} else if c.store.GetRoleStatus(utils.GetNamespaceName(ms), servingGroupName, roleName, roleID) == datastore.RoleRunning {
		...
		if c.store.GetServingGroupStatus(utils.GetNamespaceName(ms), servingGroupName) == datastore.ServingGroupRunning {
...
func (c *ModelServingController) checkServingGroupReady(ms *workloadv1alpha1.ModelServing, servingGroupName string) (bool, error) {
	...
	defer cancel()
	roles, err := c.templates(ctx, ms).groupRoleTargets(ctx, servingGroupName)
	if err != nil {
		return false, err
	}
...
func (c *ModelServingController) checkRoleReady(ms *workloadv1alpha1.ModelServing, servingGroupName, roleName, roleID string) (bool, error) {
	...
	roles, err := c.store.GetRoleList(utils.GetNamespaceName(ms), servingGroupName, roleName)
	if err != nil {
		return false, err
	}
	for _, role := range roles {
...
func (c *ModelServingController) createOrUpdatePodGroupByServingGroup(ctx context.Context, ms *workloadv1alpha1.ModelServing, servingGroupName string) error {
	groupTemplate, err := c.templates(ctx, ms).podGroupModelServing(ctx, servingGroupName)
	if err != nil {
		// Missing history must not rewrite old scheduling requirements or block
		// replica scale-down and the independently guarded rollout path.
		c.reportRevisionUnresolved(ms, servingGroupName, err)
		return nil
	}
	if err, retryAfter := c.podGroupManager.CreateOrUpdatePodGroup(ctx, groupTemplate, servingGroupName); err != nil {
		if c.recorder != nil {
			c.recorder.Eventf(
				ms,
...
func (c *ModelServingController) findOutdatedRolesInServingGroups(ms *workloadv1alpha1.ModelServing, servingGroups []datastore.ServingGroup, revision string) map[string][]string {
	...
	for _, sg := range servingGroups {
		...
		for _, desired := range ms.Spec.Template.Roles {
			...
			for _, role := range roles {
				matches, err := history.instanceMatches(ctx, sg, role, desired)
				if err != nil {
